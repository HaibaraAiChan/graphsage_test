...
class DglNodePropPredDataset(object):
    ...
    def pre_process(self):
        ...
        if osp.exists(pre_processed_file_path):
            ...
            if self.is_hetero:
                graph = read_heterograph_dgl(raw_dir, add_inverse_edge=add_inverse_edge,
                ...
                graph = \
                read_graph_dgl(raw_dir, add_inverse_edge=add_inverse_edge, additional_node_files=additional_node_files,
                               additional_edge_files=additional_edge_files, binary=self.binary)[0]
                ...
                if self.binary:
                    ...
                    node_label = pd.read_csv(osp.join(raw_dir, 'node-label.csv.gz'), compression='gzip',
                                             header=None).values
